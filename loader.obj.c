...
d_define_method_override(loader, job)(struct s_object *self) {
  ...
  struct s_object *resources_path = f_string_new_constant(d_new(string), d_linkino_resources),
    *template_png = f_string_new_constant(d_new(string), d_linkino_resources_default_png),
    *template_ttf = f_string_new_constant(d_new(string), d_linkino_resources_default_ttf),
    *template_json = f_string_new_constant(d_new(string), d_linkino_resources_default_json),
    *template_ogg = f_string_new_constant(d_new(string), d_linkino_resources_default_ogg),
    *template_lisp = f_string_new_constant(d_new(string), d_linkino_resources_default_lisp);
  d_try
      {
        ...
        if ((loader_attributes->media_factory =
               f_media_factory_new(d_new(media_factory), loader_attributes->resources_png, loader_attributes->resources_json, loader_attributes->resources_ogg,
                 loader_attributes->resources_lisp, loader_attributes->environment))) {
          d_assert(
            stream_configuration = d_call(loader_attributes->resources_json, m_resources_get_stream, d_linkino_game_configuration, e_resources_type_common));
          if ((json_configuration = f_json_new_stream(d_new(json), stream_configuration))) {
            d_assert(stream_ui = d_call(loader_attributes->resources_json, m_resources_get_stream, d_linkino_ui_configuration, e_resources_type_common));
            if ((json_ui = f_json_new_stream(d_new(json), stream_ui))) {
              if ((loader_attributes->ui_factory =
